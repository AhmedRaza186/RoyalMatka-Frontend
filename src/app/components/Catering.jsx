...
export default function Catering() {
  return (
    <section
      id="catering"
      className="relative overflow-hidden bg-[#131110] py-20 md:py-28 border-t border-brand-gold/10"
    >
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-3 lg:mt-16">
          {CATERING_FEATURES.map((item) => (
            <div
              key={item.title}
              className="group rounded-2xl border border-brand-gold/10 bg-brand-charcoal/60 p-7 text-center transition-all duration-300 hover:-translate-y-1 hover:border-brand-gold/35 hover:bg-brand-charcoal hover:shadow-xl hover:shadow-brand-gold/5"
            >
              {/* Icon */}
              ...
              <h3 className="mt-5 font-serif text-2xl font-semibold text-brand-cream">
                {item.title}
              </h3>

              <p className="mt-3 font-sans text-sm leading-6 text-neutral-muted">
                {item.description}
              </p>
            </div>
          ))}
        ...
        {/* Bottom CTA Card */}
        <div className="mt-12 rounded-3xl border border-brand-gold/15 bg-gradient-to-br from-brand-charcoal to-[#161311] p-8 text-center shadow-2xl md:mt-16 md:p-12 hover:border-brand-gold/25 transition-colors duration-350">
          <div className="mx-auto max-w-2xl">
            <p className="font-sans text-xs font-bold uppercase tracking-[0.2em] text-brand-gold">
              Planning an Event?
            </p>

            <h3 className="mt-3 font-serif text-3xl font-bold text-brand-cream sm:text-4xl leading-tight">
              Let&apos;s Make Your Celebration Special
            </h3>

            <p className="mt-4 font-sans text-sm leading-6 text-neutral-muted">
